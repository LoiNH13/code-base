    ...
    where TEntity : Entity
{
    ...
    /// </summary>
    /// <returns></returns>
    public virtual IQueryable<TEntity> Queryable() => DbContext.Set<TEntity>().AsQueryable();

    /// <summary>
    /// Gets the split queryable.
    /// </summary>
    /// <returns></returns>
    public IQueryable<TEntity> QueryableSplitQuery() => DbContext.Set<TEntity>().AsQueryable().AsSplitQuery();
    ...
    protected async Task<Maybe<TEntity>> FirstOrDefaultAsync(Specification<TEntity> specification) =>
        await DbContext.Set<TEntity>().FirstOrDefaultAsync(specification) ?? default!;

    /// <summary>
    /// Checks if the entity is attached to the DbContext.
    /// </summary>
    /// <typeparam name="TEntity">The type of entity to check</typeparam>
    /// <param name="entity">The entity to check</param>
    /// <returns>True if the entity is attached, false if detached</returns>
    protected bool IsEntityAttached(TEntity entity)
    {
        var entityEntry = ((DbContext)DbContext).Entry(entity);
